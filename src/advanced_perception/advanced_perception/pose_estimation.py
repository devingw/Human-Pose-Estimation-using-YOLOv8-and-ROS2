...
class PoseEstimationNode(Node):
    ...
    def callback(self, data: Image) -> None:
        ...
        '''Performs pose estimation by extracting the detected keypoints and publishes the annotated image for visualisation.
           It also calculates the angle between specified keypoints for further analysis'''

        img = self.bridge.imgmsg_to_cv2(data, "bgr8")
        results = self.model(img)

        #Initialising the PoseResult message 

        pose_result = PoseResult()
        pose_result.header.stamp = self.get_clock().now().to_msg()
        pose_result.header.frame_id = "pose_estimation"

        #Checking if results are valid and have keypoints

        if results and hasattr(results[0], "keypoints") and results[0].keypoints.conf is not None:
            #Extracting keypoints and thier confidence values
